...
from PyQt5.QtWidgets import (QDialog, QWidget, QVBoxLayout, QHBoxLayout, QFormLayout,
    QLabel, QLineEdit, QPushButton, QFileDialog, QDialogButtonBox, QScrollArea, QFrame)
...
def resolve_dep_path(stored: str) -> str:
    ...
    if os.path.isabs(stored): return stored
...
def discover_dep(dep: dict) -> str:
    base = _base_dir()
    names = dep["candidates_win"] if platform.system() == "Windows" else (dep["candidates_mac"] if platform.system() == "Darwin" else dep["candidates_lin"])
    
    for name in names:
        if os.path.isfile(os.path.join(base, name)): return os.path.join(base, name)
    
    if dep["is_library"]:
        search_dirs = ["/usr/lib", "/usr/local/lib", "/usr/lib/x86_64-linux-gnu", "/usr/lib64", "/opt/homebrew/lib", "/opt/local/lib"]
...
def check_and_store_deps(settings) -> list:
    ...
    for dep in DEP_DEFINITIONS:
        ...
        if stored and os.path.exists(resolve_dep_path(stored)): continue
        found = discover_dep(dep)
        if found: settings.setValue(dep["key"], _make_stored_path(found))
...
class DepsTab(QWidget):
    ...
    def __init__(self, parent, settings, missing_only=False):
        ...
        for dep in DEP_DEFINITIONS:
            ...
            if missing_only and current and os.path.exists(resolve_dep_path(current)): continue
            ...
            edit.textChanged.connect(lambda t, e=edit: e.setStyleSheet("" if os.path.exists(t) else "border: 2px solid red;"))
            edit.textChanged.emit(edit.text())
            btn = QPushButton(self.tr("Browse..."))
            btn.clicked.connect(lambda _, d=dep, e=edit: e.setText(QFileDialog.getOpenFileName(self, self.tr(f"Locate {d['name']}"), "", d["browse_filter"])[0] or e.text()))
            layout.addWidget(edit, 1); layout.addWidget(btn)
            
            self._rows[dep["key"]] = edit
            lbl = QLabel(f"<b>{dep['name']}</b><br/><small>{dep['description']}</small>"); lbl.setTextFormat(Qt.RichText)
            form.addRow(lbl, row)
    ...
    def _rediscover(self):
        for dep in DEP_DEFINITIONS:
            found = discover_dep(dep)
            if found and dep["key"] in self._rows: self._rows[dep["key"]].setText(found)

    def save(self):
        for dep in DEP_DEFINITIONS:
            if dep["key"] in self._rows:
                val = self._rows[dep["key"]].text().strip()
                if val and os.path.exists(val): self.settings.setValue(dep["key"], _make_stored_path(val))
                elif not val: self.settings.remove(dep["key"])
...
class DepsDialog(QDialog):
    def __init__(self, parent, settings, missing_only=True):
        ...
        layout.addWidget(QLabel(self.tr("<b>Some required external tools could not be found automatically.</b><br>Please locate them below.")))
