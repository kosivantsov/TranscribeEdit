...
class HandyTab(QWidget):
    TAB_LABEL = "Handy Tool"
    ...
    def __init__(self, parent, settings):
        ...
        info = QLabel(self.tr("Configure the local CLI transcription tool (Handy Tool), including the path to the executable and the active inference model."))
    ...
    def refresh_info(self, silent=False):
        ...
        if not bin_path or not os.path.isfile(bin_path):
            if not silent: QMessageBox.warning(self, self.tr("Error"), self.tr("Invalid binary path."))
            return
        ...
        try:
            res = subprocess.run([bin_path, "--list-models", "--json"], capture_output=True, text=True, encoding="utf-8", errors="replace")
            ...
            if not silent: print(f"Failed to fetch models: {e}")

        try:
            res = subprocess.run([bin_path, "--list-devices"], capture_output=True, text=True, encoding="utf-8", errors="replace")
            for line in res.stdout.splitlines():
                if line.strip().startswith("index="):
                    ...
                    if idx_match and name_match:
                        self.device_combo.addItem(f"[{kind_match.group(1) if kind_match else 'unknown'}:{idx_match.group(1)}] {name_match.group(1).strip()}", idx_match.group(1))

            idx = self.device_combo.findData(self._current_device)
            if idx >= 0: self.device_combo.setCurrentIndex(idx)
            if not silent: QMessageBox.information(self, self.tr("Success"), self.tr("Successfully refreshed info."))
        except Exception as e:
            if not silent: print(f"Failed to fetch devices: {e}")
        finally:
    ...
    def save(self):
        ...
        if midx >= 0: self.settings.setValue("handy_model", self.model_combo.itemData(midx))
        didx = self.device_combo.currentIndex()
        if didx >= 0: self.settings.setValue("handy_device", self.device_combo.itemData(didx))
